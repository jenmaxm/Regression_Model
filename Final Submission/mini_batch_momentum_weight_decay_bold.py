...
class NeuralNetwork:
    ...
    def backward(self, X, y, learning_rate):
        ...
        # Compute gradients
        grad_wih = X.T.dot(hidden_delta) + lambda_wd * self.weights_input_hidden  # Apply weight decay separately
        grad_bh = np.sum(hidden_delta, axis=0, keepdims=True)  # No weight decay for bias
        grad_who = self.hidden_layer.T.dot(output_delta) + lambda_wd * self.weights_hidden_output
        grad_bo = np.sum(output_delta, axis=0, keepdims=True)  # No weight decay for bias
...
def train_and_predict(file_path):
    ...
    Y_test_pred_scaled = nn.forward(X_test)  # Predictions in the scaled space
