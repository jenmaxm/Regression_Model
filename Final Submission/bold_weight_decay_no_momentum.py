...
class NeuralNetwork:
    ...
    def backward(self, X, y, learning_rate):
        ...
        grad_wih = X.T.dot(hidden_delta) + self.weight_decay * self.weights_input_hidden
        grad_bh = np.sum(hidden_delta, axis=0, keepdims=True)  
        grad_who = self.hidden_layer.T.dot(output_delta) + self.weight_decay * self.weights_hidden_output
    ...
    def train(self, X_train, y_train_scaled, X_valid, y_valid_scaled, scaler_Y, epochs, learning_rate):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100 == 0:
                y_pred_train = scaler_Y.inverse_transform(self.output_layer)
                y_train_real = scaler_Y.inverse_transform(y_train_scaled)
                train_error = rmse_loss(y_train_real, y_pred_train)

                # Compute validation loss
                y_pred_valid = scaler_Y.inverse_transform(self.forward(X_valid))
                y_valid_real = scaler_Y.inverse_transform(y_valid_scaled)
                valid_error = rmse_loss(y_valid_real, y_pred_valid)

                train_loss_history.append(train_error)
                valid_loss_history.append(valid_error)

                print(f"Epoch {epoch}, Train RMSE: {train_error:.6f}, Valid RMSE: {valid_error:.6f}, LR: {learning_rate:.6f}")
...
def plot_loss_vs_epochs(train_loss, valid_loss):
    plt.figure(figsize=(8, 5))
    epochs = np.arange(0, len(train_loss) * 100, 100)
    plt.plot(epochs, train_loss, label="Training RMSE", color='b')
    plt.plot(epochs, valid_loss, label="Validation RMSE", color='r', linestyle='--')
    plt.xlabel('Epochs')
...
def train_and_predict(file_path):
    ...
    nn = NeuralNetwork(input_size=input_size, hidden_size=12, output_size=1, weight_decay=0.0001)

    train_loss, valid_loss = nn.train(X_train, Y_train_scaled, X_valid, Y_valid_scaled, scaler_Y, epochs=40000, learning_rate=0.1)
    
    plot_loss_vs_epochs(train_loss, valid_loss)
