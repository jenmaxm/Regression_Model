...
class NeuralNetwork:
    ...
    def train(self, X_train, y_train_scaled, X_valid, y_valid_scaled, scaler_Y, epochs, learning_rate, batch_size=16):
        prev_loss = float('inf')
        loss_history = []
...
def train_and_predict(file_path):
    ...
    nn = NeuralNetwork(input_size=input_size, hidden_size=8, output_size=1, momentum=0.85)

    # Train the network with validation loss tracking
    nn.train(X_train, Y_train_scaled, X_valid, Y_valid_scaled, scaler_Y, epochs= 10000, learning_rate=0.1)
