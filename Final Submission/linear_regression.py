...
class NeuralNetwork:
    def __init__(self, input_size, hidden_size, output_size, momentum=0.85):
    ...
    def train(self, X_train, y_train_scaled, X_valid, y_valid_scaled, scaler_Y, epochs, learning_rate, batch_size=32):
        prev_loss = float('inf')
        loss_history = []
...
def train_baseline_linear_regression(X_train, Y_train, X_valid, Y_valid, X_test, Y_test):
    """
    Trains a baseline Linear Regression model and evaluates it on the validation and test sets.
    """
    ...
    print(f"Train RMSE: {train_rmse:.6f}, Validation RMSE: {valid_rmse:.6f}, Test RMSE: {test_rmse:.6f}")
...
def train_and_predict(file_path):
    ...
    linear_model, Y_test_pred_lr = train_baseline_linear_regression(X_train, Y_train, X_valid, Y_valid, X_test, Y_test)
    
    # Initialize and train the neural network
    input_size = X_train.shape[1]  # Number of features
    nn = NeuralNetwork(input_size=input_size, hidden_size=16, output_size=1, momentum=0.85)
    nn.train(X_train, Y_train_scaled, X_valid, Y_valid_scaled, scaler_Y, epochs=7500, learning_rate=0.1, batch_size=32)
    
    # Neural network predictions
    Y_pred_scaled_nn = nn.forward(X_test)
    Y_test_pred_nn = scaler_Y.inverse_transform(Y_pred_scaled_nn)
    ...
    plt.scatter(range(len(Y_test)), Y_test, label='Actual', marker='.', alpha=0.8, color='dodgerblue', s=60)
    plt.scatter(range(len(Y_test_pred_nn)), Y_test_pred_nn, label='Neural Network', marker='.', alpha=0.8, color='crimson', s=60)
    plt.scatter(range(len(Y_test_pred_lr)), Y_test_pred_lr, label='Linear Regression', marker='.', alpha=0.8, color='green', s=60)
    
    plt.xlabel('Sample Index', fontsize=12)
    plt.ylabel('Target Value', fontsize=12)
    plt.title('Neural Network vs. Linear Regression Predictions', fontsize=14, fontweight='bold')
    plt.legend(fontsize=12)
    plt.grid(True)
    plt.show()
