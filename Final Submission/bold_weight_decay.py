...
#0.8963 correlation 0.9 to 2 s.f , 
...
class NeuralNetwork:
    def __init__(self, input_size, hidden_size, output_size, momentum=0.85, weight_decay=0.001):
    ...
    def backward(self, X, y, learning_rate):
        ...
        grad_wih = X.T.dot(hidden_delta) + self.weight_decay * self.weights_input_hidden  # L2 Regularization
        grad_bh = np.sum(hidden_delta, axis=0, keepdims=True)  
        grad_who = self.hidden_layer.T.dot(output_delta) + self.weight_decay * self.weights_hidden_output  # L2 Regularization
    ...
    def train(self, X_train, y_train_scaled, X_valid, y_valid_scaled, scaler_Y, epochs, learning_rate):
        ...
        # Lists to track training and validation errors for plotting
        train_errors = []
        valid_errors = []
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100 == 0:
                # Get predicted values on the training and validation set (scaled)
                y_pred_train_real = scaler_Y.inverse_transform(self.output_layer)
                y_train_real = scaler_Y.inverse_transform(y_train_scaled)

                self.forward(X_valid)
                y_pred_valid_real = scaler_Y.inverse_transform(self.output_layer)
                y_valid_real = scaler_Y.inverse_transform(y_valid_scaled)

                # Calculate RMSE loss
                train_error = rmse_loss(y_train_real, y_pred_train_real)
                valid_error = rmse_loss(y_valid_real, y_pred_valid_real)
                print(f"Epoch {epoch}, Train Error: {train_error:.6f}, Valid Error: {valid_error:.6f}, LR: {learning_rate:.6f}")
                
                train_errors.append(train_error)
                valid_errors.append(valid_error)

                # Adjust the learning rate based on previous loss
                if valid_error < prev_loss:
                    learning_rate *= 1.05  
                else:
                    learning_rate *= 0.7

                # Update prev_loss to the current validation error for the next iteration
                prev_loss = valid_error
...
def plot_error_vs_epochs(train_errors, valid_errors):
    """
    Plots the RMSE error against epochs for both training and validation sets.
    """
    plt.figure(figsize=(8, 6))
    epochs_range = range(0, len(train_errors) * 100, 100)
    plt.plot(epochs_range, train_errors, color='blue', label="Training Error")
    plt.plot(epochs_range, valid_errors, color='red', linestyle='dashed', label="Validation Error")
...
def train_and_predict(file_path):
    ...
    # Initialize the neural network with weight decay
    input_size = X_train.shape[1]  # Number of features
    nn = NeuralNetwork(input_size=input_size, hidden_size=12, output_size=1, weight_decay=0.001)  # Set weight decay

    # Train the network with validation loss tracking
    train_errors, valid_errors = nn.train(X_train, Y_train_scaled, X_valid, Y_valid_scaled, scaler_Y, epochs=20000, learning_rate=0.1)

    # Plot the predictions
    plot_predictions_and_correlation(nn, X_test, Y_test_scaled, scaler_Y)

    # Plot Training vs Validation Error
    plot_error_vs_epochs(train_errors, valid_errors)
